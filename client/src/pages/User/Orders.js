...
function Orders() {
    ...
    useEffect(() => {
        if (Array.isArray(orders)) {
            ...
            setStoredOrders(ordersFromLocalStorage);
        }
    }, [orders]);

    const handleCancelOrder = (orderIndex, productIndex) => {
        const handleCancelConfirmation = () => {
            const confirmation = window.confirm('Bạn có chắc chắn muốn hủy đơn hàng này không?');
            if (confirmation) {
                const updatedOrders = [...storedOrders];
                updatedOrders[orderIndex].products.splice(productIndex, 1);
                localStorage.setItem('orders', JSON.stringify(updatedOrders)); // Cập nhật dữ liệu trong localStorage
    ...
    return (
        <DefaultLayout title={'Đơn hàng | Sopi VN'}>
            <div className={cx('app_container')}>
                <div className={cx('grid')}>
                    <div className={cx('tablist')}>
                        <div className={cx('tabitem')}>
                            <div className={cx('tablink')}>Tất cả</div>
                        </div>
                    </div>
                    {storedOrders.map((order, orderIndex) => (
                        <div key={orderIndex}>
                            <>
                                <div className={cx('address')}>
                                    <div className={cx('address-item')}>
                                        <div className={cx('address-text')}>Họ và tên: </div>
                                        <div className={cx('address-link')}>{order.address.name}</div>
                                    </div>
                                    <div className={cx('address-item')}>
                                        <div className={cx('address-text')}>Số điện thoại: </div>
                                        <div className={cx('address-link')}>{order.address.phoneNumber}</div>
                                    </div>
                                    <div className={cx('address-item')}>
                                        <div className={cx('address-text')}>Địa chỉ: </div>
                                        <div className={cx('address-link')}> {order.address.address}</div>
                                    </div>
                                </div>
                                    ...
                                    <div key={productIndex}>
                                        <div className={cx('orders')}>
                                            <div className={cx('order-head')}>
                                                <div className={cx('order-head__left')}>
                                                    <svg
                                                    ...
                                                    <div className={cx('name-shop')}>GG Official</div>
                                                    <div className={cx('actions', 'chat-box')}>
                                                        <svg
                                                            viewBox="0 0 17 17"
                                                            className={cx('shopee-svg-icon', 'icon-btn-chat')}
                                                            style={{ fill: 'white' }}
                                                        >
                                                            <g fillRule="evenodd">
                                                                <path
                                                    ...
                                                    <div className={cx('actions', 'review')}>
                                                        <svg
                                                            enableBackground="new 0 0 15 15"
                                                            viewBox="0 0 15 15"
                                                            x="0"
                                                            y="0"
                                                            className={cx('shopee-svg-icon', 'icon-btn-shop')}
                                                        >
                                                        ...
                                                        <span className={cx('review-text')}>Xem shop</span>
                                                    </div>
                                                </div>
                                                <div className={cx('order-head__right')}>
                                                    <svg
                                                        enableBackground="new 0 0 15 15"
                                                        viewBox="0 0 15 15"
                                                        x="0"
                                                        y="0"
                                                        className={cx('shopee-svg-icon', 'icon-free-shipping-line')}
                                                    >
                                                        <g>
                                                            <line
                                                                fill="none"
                                                    ...
                                                    <span className={cx('shipping')}>Đang vận chuyển</span>
                                                </div>
                                            </div>

                                            <div className={cx('order-product')}>
                                                <div className={cx('order-product__item')}>
                                                    <img
                                                        src={product.image}
                                                        alt={product.name}
                                                    ...
                                                    <div className={cx('order-product__info')}>
                                                        <div className={cx('order-product__name')}>{product.name}</div>
                                            ...
                                            <div className={cx('price')}>
                                                ...
                                                <span className={cx('total-text')}>Thành tiền: </span>
                                                <span className={cx('total')}>₫{product.totalPrice}</span>
                                            </div>

                                            <div className={cx('order-btn')}>
                                                <button
                                                    className={cx('btn', 'btn--primary')}
                                                    onClick={() => handleCancelOrder(orderIndex, productIndex)}
                                                >
                                                    Hủy
                                                </button>
                                                <button className={cx('btn', 'btn--lh')}>Liên Hệ Người Bán</button>
                                            </div>
                                        </div>
                                    </div>
                                ))}
